...
import { isTranslationServerReady, subscribeTranslationServerReady } from "../../utils/serverReady";
...
export function AppShell() {
  ...
  useEffect(() => { let cancelled = false; void (async () => { try { const [dbPapers, dbLinks, dbNodes] = await Promise.all([getAllPapers(), getAllWorkspacePapers(), listWorkspace()]); if (cancelled) return; mergeWorkspaceNodes(dbNodes); setMemberships(dbLinks); setPapers(dbPapers); for (const paper of dbPapers) { const [sections, blocks] = await Promise.all([getSectionsByPaper(paper.id), getBlocksByPaper(paper.id)]); if (cancelled) return; setSections(paper.id, (prev) => mergePreferTranslatedSections(prev, sections)); setBlocks(paper.id, (prev) => mergePreferTranslated(prev, blocks)); const status = displayProcessingStatus(paper.processingStatus, blocks, (block) => isRetryableTranslationFailure(block)); if (status !== paper.processingStatus) { await savePaper({ ...paper, processingStatus: status }); updatePaper(paper.id, { processingStatus: status }); } } if (!cancelled && isTranslationServerReady()) { resumeAfterTranslationServerReady(); } } catch (error) { console.error("Failed to load library:", error); } finally { if (!cancelled) setLoaded(true); } })(); const unsubscribe = subscribeTranslationServerReady(() => { if (!cancelled) resumeAfterTranslationServerReady(); }); return () => { cancelled = true; unsubscribe(); }; }, [mergeWorkspaceNodes, setBlocks, setLoaded, setMemberships, setPapers, setSections, updatePaper]);
