...
> Version 0.2 / 2026-09-04  
> プロダクト原則と画面方針の基準文書  
> 起動手順や現行スタックは [README.md](./README.md) を正とする

## 実装状況（2026-09-04）

SPEC の原則はそのまま残し、実装は次まで進んでいる。

- Tauri 2 の macOS `.app`。翻訳サーバーはサイドカー起動
- Project / Inbox / 常設サイドバー（ホバーで展開）
- MADLAD-400 3B + MPS + bfloat16、文バッチ
- スキャン PDF は Apple Vision OCR（Tauri のみ）
- PDF 抽出は pdf.js 3.11（6.x は WKWebView 非対応）
- 用語集は Ollama（任意）

未着手のままなのは、図中文字の翻訳、注釈、クラウド同期、Windows、App Store 配布など。
...
- 図表画像内 OCR と図中文字の翻訳（ページ全体のスキャン OCR は実装済み）
...
すでに採用しているもの: React + Zustand、pdf.js 3.11、MADLAD 3B on MPS、Tauri 2 + FastAPI サイドカー、IndexedDB、文バッチ翻訳。

技術上の都合だけで本書のプロダクト原則（日本語主役、ローカルファースト、途中から読める）は変えない。
