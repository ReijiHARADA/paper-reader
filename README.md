...
- **途中から読む**: PDF 追加後は Library のカードが「準備中」→「読めます / 日本語化中」になる。構造解析が終わった時点でリーダーを開ける。論文を開くと `lastOpenedAt` を記録する。翻訳はタイトル → 見出し → Abstract → 本文の順。いま読んでいる付近を優先して訳す。未完了や翻訳サーバー停止で失敗した段落は、次に論文を開いたときに再開する。サーバー未接続で import 前に止まったカードには再試行と削除を表示し、部分保存済みの「要確認」論文は三点メニューから翻訳を再試行できる
- **メモ**: 訳文を選択すると近くに入力 popover が出る。その場で保存すると黄色ハイライトになる。ハイライトをクリックすると同じ場所で内容を確認・編集・削除できる。削除は Undo できる。右の Notes は論文全体の一覧と、一覧から本文位置への移動だけに使う。再翻訳で位置がずれたメモは orphan 扱い
...
- **Markdown 書き出し**: リーダー右上の書き出しから、きれいな Markdown または内部 block comment 付きの検証用 Markdown、検証用パッケージ（`source.pdf` + `translated.md` + `assets/`）、Notion インポート用 ZIP を選べる。Notion 用 ZIP は YAML front matter を除き、Markdown と画像を ZIP の直下に置き、画像リンクを平坦化する。キャプションは画像の直後の本文として残すため、余分な `assets` ページを作らず図表画像とキャプションも一緒に追加される。Paper Package を開き直しても `assets/` の画像とキャプションは Reader に復元される。翻訳失敗箇所の含める／除くも選べる。Tauri では `.md` の隣に `assets/` を書き、browser `npm run dev` は画像があるとき zip にする。ダイアログはアプリのレイアウトから分離して表示し、狭いウィンドウでも内容をスクロールできる。書き出し中でなければ Escape または背景のクリックでも閉じられる。Reader と export は同じ Markdown / Document AST
...
- カードの `…` からワークスペースへ追加、お気に入り、ライブラリ削除ができます。お気に入り中の論文はカードタイトル横に星が出ます
